  ...
  onPreUpdate(engine) {
    ...
    if (this.charselect == 0) {
      switch (this.playerId) {
        case 1:
          if (
            kb.isHeld(Input.Keys.W) ||
            kb.isHeld(Input.Keys.Up) ||
            controller.at(0).getAxes(Input.Axes.LeftStickY) < -0.5
          ) {
            yspeed = -300;
          }
          if (
            kb.isHeld(Input.Keys.S) ||
            kb.isHeld(Input.Keys.Down) ||
            controller.at(0).getAxes(Input.Axes.LeftStickY) > 0.5
          ) {
            yspeed = 300;
          }
          if (
            kb.isHeld(Input.Keys.A) ||
            kb.isHeld(Input.Keys.Left) ||
            controller.at(0).getAxes(Input.Axes.LeftStickX) < -0.5
          ) {
            xspeed = -300;
          }
          if (
            kb.isHeld(Input.Keys.D) ||
            kb.isHeld(Input.Keys.Right) ||
            controller.at(0).getAxes(Input.Axes.LeftStickX) > 0.5
          ) {
            xspeed = 300;
          }
        ...
        case 2:
          if (
            kb.isHeld(Input.Keys.I) ||
            kb.isHeld(Input.Keys.Up) ||
            controller.at(1).getAxes(Input.Axes.LeftStickY) < -0.5
          ) {
            yspeed = -300;
          }
          if (
            kb.isHeld(Input.Keys.K) ||
            kb.isHeld(Input.Keys.Down) ||
            controller.at(1).getAxes(Input.Axes.LeftStickY) > 0.5
          ) {
            yspeed = 300;
          ...
          if (
            kb.isHeld(Input.Keys.J) ||
            kb.isHeld(Input.Keys.Left) ||
            controller.at(1).getAxes(Input.Axes.LeftStickX) < -0.5
          ) {
            xspeed = -300;
          }
          if (
            kb.isHeld(Input.Keys.L) ||
            kb.isHeld(Input.Keys.Right) ||
            controller.at(1).getAxes(Input.Axes.LeftStickX) > 0.5
          ) {
            xspeed = 300;
      ...
      this.pos.x = clamp(
        this.pos.x,
        -1050,
        Resources.realLevel.width * 1.84 - this.width / 2
      );
      this.pos.y = clamp(
        this.pos.y,
        -460,
        Resources.realLevel.height * 1.633 - this.height / 2
      );
    } else {
      switch (this.playerId) {
        case 1:
          if (this.selectedP1 != 1) {
            if (
              kb.wasPressed(Input.Keys.W) ||
              kb.wasPressed(Input.Keys.Up) ||
              controller.at(0).getAxes(Input.Axes.LeftStickY) < -0.5
            ) {
              if (this.charId >= 2) {
                this.charId = -1;
            ...
            if (
              kb.wasPressed(Input.Keys.S) ||
              kb.wasPressed(Input.Keys.Down) ||
              controller.at(0).getAxes(Input.Axes.LeftStickY) > 0.5
            ) {
              if (this.charId == 0) {
                this.charId = 1;
